...
def find_token():
    ...
    for token in R.see():
    	if token.dist < dist:
        	if token.info.marker_type == MARKER_TOKEN_GOLD:
        		closest_token_type = 'gold'
        		dist = token.dist
        		rot_y = token.rot_y
        	elif token.info.marker_type == MARKER_TOKEN_SILVER:
        		closest_token_type = 'silver'
        		dist = token.dist
        		rot_y = token.rot_y
	...
	return -1, -1, -1
   	...
   	return dist, rot_y, closest_token_type
   	
 

while 1:
    dist, rot_y,closest_token_type = find_token()  # we look for markers
    if closest_token_type == 'gold':
    	if dist==-1:
        	print("I don't see any token!!")
		drive(50,3)  # if no markers are detected
    ...
    if closest_token_type == 'silver':
    	
    	if dist==-1:
        	print("I don't see any token!!")
		drive(50,3)  # if no markers are detected  # if no markers are detected, the program ends
